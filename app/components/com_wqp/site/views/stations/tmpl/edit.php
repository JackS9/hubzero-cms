...
Document::setTitle(Lang::txt('COM_WQP') . ': ' . Lang::txt('COM_WQP_STATIONS') . ': ' . ($this->entry->get('id') ? Lang::txt('JACTION_EDIT') : Lang::txt('JACTION_NEW')));
...
<section class="main section">
	...
	<form action="<?php echo Route::url('index.php?option=' . $this->option . '&controller=' . $this->controller . '&task=save'); ?>" method="post" id="hubForm">
		...
		<fieldset>
			...
			<label for="field-monitoring_location_identifier">
				<?php echo Lang::txt('COM_WQP_MONITORING_LOCATION_IDENTIFIER'); ?> <span class="required"><?php echo Lang::txt('JOPTION_REQUIRED'); ?></span>
				<input type="text" name="entry[MonitoringLocationIdentifier]" id="field-monitoring_location_identifier" size="35" value="<?php echo $this->escape($this->entry->get('MonitoringLocationIdentifier')); ?>" />
			</label>

			<div class="grid">
				<div class="col span6">
					<label for="field-latitude_measure">
						<?php echo Lang::txt('COM_WQP_LATITUDE_MEASURE'); ?>
						<input type="text" name="entry[LatitudeMeasure]" id="field-latitude_measure" value="<?php echo $this->escape($this->entry->get('LatitudeMeasure')); ?>" />
					</label>
				</div>

				<div class="col span6 omega">
					<label for="field-longitude_measure">
						<?php echo Lang::txt('COM_WQP_LONGITUDE_MEASURE'); ?>
						<input type="text" name="entry[LongitudeMeasure]" id="field-longitude_measure" value="<?php echo $this->escape($this->entry->get('LongitudeMeasure')); ?>" />
					</label>
				</div>
			</div>
		</fieldset>
