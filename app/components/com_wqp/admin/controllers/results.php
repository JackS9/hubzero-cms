<?php
namespace Components\Wqp\Admin\Controllers;
...
class Results extends AdminController
{
	...
	public function displayTask()
	{
		...
		$this->view->filters = array(
			...
			'station' => urldecode(Request::getState(
				$this->_option . '.' . $this->_controller . '.station',
				'station',
				0
			)),
		...
		{
			$record->whereLike('CharacteristicName', $search);
		}
	...
	public function saveTask()
	{
		...
		$row = Result::oneOrNew($fields['id'])->set($fields);
		...
		Notify::success(Lang::txt('COM_WQP_ENTRY_SAVED'));
	...
	public function removeTask()
	{
		...
		if (count($ids) > 0)
		{
			...
			foreach ($ids as $id)
			{
				...
				$entry = Result::oneOrFail(intval($id));
		...
		App::redirect(
			Route::url('index.php?option=' . $this->_option . '&controller=' . $this->_controller, false),
			($removed ? Lang::txt('COM_WQP_ENTRIES_DELETED') : null)
		);
	}
	...
	public function stateTask()
	{
		...
		if (count($ids) < 1)
		{
			...
			App::redirect(
				Route::url('index.php?option=' . $this->_option . '&controller=' . $this->_controller, false),
				Lang::txt('COM_WQP_SELECT_ENTRY_TO', $this->_task),
				'error'
			);
			return;
		}
		...
		foreach ($ids as $id)
		{
			...
			$row = Result::oneOrNew(intval($id))->set(array('state' => $state));
		...
		{
			case 'publish':
				$message = Lang::txt('COM_WQP_ITEMS_PUBLISHED', $success);
			break;
			case 'unpublish':
				$message = Lang::txt('COM_WQP_ITEMS_UNPUBLISHED', $success);
			break;
			case 'archive':
				$message = Lang::txt('COM_WQP_ITEMS_ARCHIVED', $success);
			break;
		}
